...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        a_nodes = set()
        for _action in self.all_actions:
            a_node = PgNode_a(_action)
    ...
    def add_literal_level(self, level):
        ...
        if level == 0:
            raise Exception('S0 is already created')
        s_nodes = set()
    ...
    def h_levelsum(self) -> int:
        ...
        for this_goal in self.problem.goal:
            this_goal_found = False
            for current_depth in range(len(self.s_levels)):
                for state in self.s_levels[current_depth]:
                    literal_of_state = expr(state.symbol)
                    if not state.is_pos:
                        literal_of_state = '~' + literal_of_state
                    if literal_of_state == this_goal:
                        this_goal_found = True
                        level_sum += current_depth
                        break
                if this_goal_found:
                    break
        return level_sum